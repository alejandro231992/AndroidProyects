...
class LocaleManager(context: Context) {
    ...
    val language: String?
        get() = getSharedPrefInstance().getStringValue(LANGUAGE_KEY, LANGUAGE_SPANISH)
    ...
    private fun updateResources(context: Context, language: String?): Context {
        ...
        var configt = Configuration(res.configuration)
        context = if (Build.VERSION.SDK_INT >= N) {
            setLocaleForApi24(configt, locale)
            context.createConfigurationContext(configt)
        } else {
            configt.setLocale(locale)
            context.createConfigurationContext(configt)
        }
        return context
    }
